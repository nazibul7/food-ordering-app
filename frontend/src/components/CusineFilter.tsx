...
export default function CusineFilter({
  selectedCusine,
  onChange,
  isExpandes,
  onExpandedClick,
  ...
  const handleCusineChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    const newCusineList = event.target.checked
      ? [...selectedCusine, clickCusine]
      : selectedCusine.filter((cusine) => cusine != clickCusine);
            ...
            return (
              <div key={index} className="flex">
                ...
                <Label
                  ...
                  className={`flex items-center text-sm rounded-full flex-1 px-4 py-2 font-semibold ${isSelected ? "border border-green-600 text-green-600" : "border border-slate-300"}`}
                >
