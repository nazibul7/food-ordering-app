...
export const useCreateResturant = () => {
  ...
  const creatResturantRequest = async (
  ...
  const {
    mutateAsync: creatResturant,
    isLoading,
    isSuccess,
    error,
  } = useMutation(creatResturantRequest, {
    onSuccess: () => {
...
export const useGetresturantOrders = () => {
  ...
  const { data: orderForResturantOwner, isLoading } = useQuery(
    "resturantOrder",
    resturantOrderResuest
  );
...
export const useUpdateResturantOrder = () => {
  const { getAccessTokenSilently } = useAuth0();
  const updateResturantOrder = async (
    updateStatusOrder: TUpdateOrderStatusRequest
  ) => {
    try {
      const accessToken = await getAccessTokenSilently();
      const response = await fetch(
        `${API_BASE_URL}/api/resturant/order/${updateStatusOrder.orderId}/status`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${accessToken}`,
  ...
  const {
    mutateAsync: updateResturantOrderByOwner,
    isLoading,
    reset,
    isError,
    isSuccess,
  } = useMutation(updateResturantOrder);
  if (isSuccess) {
