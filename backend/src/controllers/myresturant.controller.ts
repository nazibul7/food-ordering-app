...
export const createResturant = async (
  ...
  try {
    ...
    const parsedBody = {
      ...
      menuItems: req.body.menuItems.map(
...
export const getResturant = async (
  ...
  try {
    ...
    if (!resturant) {
      return res
        .status(404)
        .json("No resturant found, Fill the form to create a resturant");
    }
...
export const updateResturant = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    const parsedBody = {
      ...
      deliveryPrice: req.body?.deliveryPrice
        ? parseFloat(req.body.deliveryPrice)
        : undefined, // Corrected conditional parsing
      estimatedDeliveryTime: req.body?.estimatedDeliveryTime
        ? parseFloat(req.body.estimatedDeliveryTime)
        : undefined, // Conditionally parse
      menuItems: req.body?.menuItems
        ? req.body.menuItems.map((item: { name: string; price: string }) => ({
            ...item,
...
export const updateOrderStatus = async (req: Request, res: Response) => {
  try {
    ...
    return res.status(500).json(`Unable to update order status ${error}`);
  }
};
